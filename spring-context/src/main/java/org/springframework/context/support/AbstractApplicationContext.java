...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			/**
			 *  获取BeanFactory;
			 *      开始创建的默认 DefaultListableBeanFactory
			 *
			...
			try {
				...
				/***
				 * 调用BeanFactory的后置处理器  在BeanFactory标准初始化之后执行
				 *
				 * 它的最重要的两个接口：
				...
				/** 子容器重写这个方法，在容器刷新的时候可以自定义逻辑 **/
				/*//空方法，在特定的子类中 初始化其他特殊bean*/
				/** {@link AbstractApplicationContext#onRefresh() } empty method to subClass implements**/
				onRefresh();

				// Check for listener beans and register them.
				// 注册并检查bean监听器
				/** 给容器注册 项目中的所有ApplicationListener **/
				//注册实现了ApplicationListener接口的监听者
				//todo 在 2.7节实例化了一个ApplicationEventMulticaster，用于管理ApplicationListener。
				//todo todo 这一节的方法就是往ApplicationEventMulticaster中注册，最终放在了一个set中。
				registerListeners();

				// Instantiate all remaining (non-lazy-init) singletons.
				//Spring开始实例化单例的类（singleton class）
				// 初始化剩下的非延迟加载（non-lazy-init）单例beans
				/** 初始化所有剩下的单实例Bean 非懒加载（non-lazy-init） **/
				...
				// 取消refresh操作，设置‘activity’标志位为false
				/** Cancel this context's refresh attempt,
				 * resetting the {@code active} flag **/
				cancelRefresh(ex);
				...
				/** clear commonCaches in Spring's core **/
				resetCommonCaches();
			}
		}
	}
	 ...
	 * @Deprecated @see org.springframework.web.context.support.WebApplicationContextUtils#initServletPropertySources
	 */
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		...
		// 这个实现执行一个上下文的底层bean工厂刷新，关闭之前已有的bean工厂，
		// 并为上下文生命周期的下一个阶段初始化bean工厂
		refreshBeanFactory();
	...
	protected void initMessageSource() {
		...
		//其实这里没必要从新获取beanFactory，直接从外边传进来不是更好嘛 (todo 可能是为了方便以后扩展容易吧)
	...
	protected void assertBeanFactoryActive() {
		if (!this.active.get()) {//default active = true
			if (this.closed.get()) {// default closed = false
